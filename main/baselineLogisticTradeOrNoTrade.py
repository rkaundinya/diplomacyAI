...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 02:57:21 2023
...
def eval_tweet_prediction(TDM, y, state = 0):
    x = TDM.T
    ...
    for ix, threshold in enumerate(np.array(range(1,1001))/1000):
        ...
        for idx in range(len(threshold_predictions)):
            if threshold_predictions[idx] == 1 and int(y_test[idx]) == 1:
                TP += 1
            if threshold_predictions[idx] == 1 and int(y_test[idx]) == 0:
                FP += 1
            if threshold_predictions[idx] == 0 and int(y_test[idx]) == 0:
                TN += 1
            if threshold_predictions[idx] == 0 and int(y_test[idx]) == 1:
                FN += 1
    ...
    print("Threshhold " +  str(max_threshold))
    holdout_predictions = classifier.predict_proba(x_test_holdout)
    
    threshold_predictions = np.array([1 if prediction[1] > max_threshold else 0 for prediction in holdout_predictions])
    
    
    for idx in range(len(threshold_predictions)):
        if threshold_predictions[idx] == 1 and int(y_test_holdout[idx]) == 1:
            TP += 1
        if threshold_predictions[idx] == 1 and int(y_test_holdout[idx]) == 0:
            FP += 1
        if threshold_predictions[idx] == 0 and int(y_test_holdout[idx]) == 0:
            TN += 1
        if threshold_predictions[idx] == 0 and int(y_test_holdout[idx]) == 1:
            FN += 1
...
def train_cbow(x, y):
    ...
    TDM, type_indexa = make_TDM(x)
    CoM, type_index = make_CoM(x)
    
    CoM_d = svdsub(CoM)
    
    CDM = cbow(TDM, CoM_d)

    result, classifier, threshold = eval_tweet_prediction(CDM, y, state = state)
...
with open('../data/tradeAndNonTradePrompts.csv', newline='') as csvfile:
    ...
    fullFile = np.loadtxt(lines, delimiter=',', quotechar="\"", skiprows=1, dtype=object)
...
result, classifier, threshold = eval_tweet_prediction(TDM, y, state = 0)

print(result)

TDM, type_index, CoM, CoM_d, CDM, result, classifier, threshold = train_cbow(x, y)
